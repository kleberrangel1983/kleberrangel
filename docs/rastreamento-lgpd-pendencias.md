...
> **Status (2026-07-12).** Doc resgatado da branch `fix/pixel-lgpd`, que nunca foi mergeada — por
> isso o bloqueador crítico abaixo ficou meses no ar sem ninguém ver. Revalidado item a item contra
> a `main` nesta data; o status de cada um está marcado no próprio item.
...
- ~~**critico — coluna.html (+ 14 páginas) — consent fica `denied` para sempre, sem banner.**~~
  **RESOLVIDO no PR #24** (`assets/consent.js` compartilhado + include nas 15 páginas), exatamente
  como o "o que fazer" abaixo recomendava. Verificado no navegador: `ad_storage` vai a `granted` no
  aceite. **Ressalva que sobrou:** `lp-dor-coluna.html` continua usando a chave divergente
  `kleber_consent` (as demais usam `drkleberrangel_consent`) — quem consente no `lp-dor-coluna` e
  navega para outra landing vê o banner de novo. Unificar a chave (lendo ambas na migração) fecha isso.
  <details><summary>Descrição original do achado</summary>
...
- **medio — index.html — banner só aparece na primeira visita, sem "Gerenciar cookies". — AINDA VÁLIDO (revalidado 2026-07-12: nenhum link "Gerenciar cookies" existe no site).**
...
- **medio — index.html — nenhuma conversão do Google Ads instrumentada. — AINDA VÁLIDO (revalidado 2026-07-12: busca por `AW-` no repo segue retornando ZERO).**
